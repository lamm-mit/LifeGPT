...
def get_model(max_length, num_words, model_name, dim=256, depth=12, heads=8, attn_dim_head=64, rot_pos=True, attn_flash=True, masking=True, mask_prob = 0.15):
    ...
    with open(model_info_file, 'w') as f:
        ...
        f.write("Note:Aug 30, 2024 - this model is a test using rot pos enc, 15 percent forgetful masking, and high entropy data on a 32 by 32 toroidal grid. Poor performance on the test set is anticipated Though, potential improvements compared to a similar model that did not use masking are expected.")

    return model, model_dir

model_name = "08_30_2024_Conway_2_State_Jump_Rot_Pos_On_Masking_On_High_Entrpoy_Homog"
rot_pos = True
model, model_dir = get_model(max_length, num_words, model_name, rot_pos=rot_pos, masking=True, mask_prob = 0.15) ##EDIT THIS FOR MODEL PARAMS
    ...
    for i in range(num_batches):
        ...
        if (i+1) % MEASURE_ACC_EVERY == 0:
            ...
            if valid_output_found and len(accuracy_list) > 0:
                ...
                accuracy = 0  # In case of failure to decode valid output in any of the test cases
